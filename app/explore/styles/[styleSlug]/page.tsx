// app/explore/styles/[styleSlug]/page.tsx
...
export default async function StyleExplorePage({ params }: PageProps) {
  // `params.styleSlug` will be something like "dali", "monet", "da-vinci", etc.
  const rawSlug = decodeURIComponent(params.styleSlug);

  const styleKey = styleSlugToKey(rawSlug);

  // If the slug isn't recognized, show a clean 404 instead of throwing.
  if (!styleKey) {
    return notFound();
  }
  ...
  // Adjust fields if your Prisma schema differs.
              ...
              return (
                ...
                >
                  <div className="relative aspect-[4/5] w-full overflow-hidden bg-muted">
                    {/* Using plain <img> to avoid extra imports;
                        safe even if URL is missing because we have a fallback. */}
                    <img
                      src={imageUrl}
                      alt={artwork.title}
                  ...
                  <div className="flex flex-1 flex-col gap-1 px-4 py-3">
                    ...
                    {artwork.artist ? (
                      <p className="text-xs text-muted-foreground">
                        by {artwork.artist.displayName ?? artwork.artist.name}
                      </p>
                    ) : null}
                  </div>
                </article>
