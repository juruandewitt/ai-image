...
# ========== Seed with diverse categories/tags ==========
...
# ========== Server helpers: build Prisma query from URL filters ==========
mkdir -p lib
...
export default function Pagination({ page, totalPages }: { page: number; totalPages: number }) {
  ...
  return (
    <div className="flex items-center justify-center gap-3 mt-6">
      <button disabled={page<=1} onClick={()=>goto(page-1)} className="px-3 py-1.5 rounded-md border disabled:opacity-40">← Prev</button>
      <span className="text-sm opacity-80">Page {page} / {totalPages}</span>
      <button disabled={page>=totalPages} onClick={()=>goto(page+1)} className="px-3 py-1.5 rounded-md border disabled:opacity-40">Next →</button>
    </div>
  )
}
TSX

# ========== Explore page: full filters + grid + pagination ==========
mkdir -p app/explore
...
# ========== Detail page: show related items ==========
...
# ========== Simple Dashboard (no auth yet): create artworks ==========
...
# ========== Ensure Prisma scripts exist ==========
node -e "const fs=require('fs');const j=JSON.parse(fs.readFileSync('package.json','utf8'));j.scripts=j.scripts||{};j.scripts['db:push']='prisma db push';j.scripts['db:seed']='tsx prisma/seed.ts';j.scripts.postinstall='prisma generate';fs.writeFileSync('package.json',JSON.stringify(j,null,2));"

echo "✔ Catalog + search feature pack written."
