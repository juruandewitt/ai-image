...
async function generateOne(style: MasterKey, title: string) {
  ...
  try {
    ...
    console.error(`✖ Bad JSON for ${title}:`, text.slice(0, 120))
    return null
  }
}
...
async function run() {
  ...
  if (!['van-gogh','rembrandt','picasso','vermeer','monet','michelangelo','dali','caravaggio','da-vinci','pollock'].includes(style)) {
    console.error('Usage: pnpm tsx scripts/generate-batch.ts <master>\nmasters: van-gogh, rembrandt, picasso, vermeer, monet, michelangelo, dali, caravaggio, da-vinci, pollock')
    process.exit(1)
  }
